...
pub fn get_closest_wall_dist(walls: &Query<(&Wall, &Transform, Without<Boid>)>, from_pos: Vec3) -> f32 {
    ...
    for (wall, mut transform, _) in walls.iter() {
        let dist = get_dist_sqr(from_pos, transform.translation).sqrt()-WALL_SIZE*2.0;
        if dist < closest {
            closest = dist;
        }
    }

    // let mut wall_size = 0.0;
    // if !walls.is_empty() {
    //     for (wall, _, _) in walls.iter() {
    //         wall_size = wall.size;
    //         break;
    //     }
    // }
...
//
// fn get_dist_sqr(pos1: Vec3, pos2: Vec3) -> f32{
//     let a = pos1.x-pos2.x;
//     let b = pos1.y-pos2.y;
//     return a*a+b*b;
// }